...
//===========================================================================
//
class ContactValueSubtype extends CAO {
    ...
    function Get($key)
    {
        $sql = "select * from t_contactvaluesubtypes where ";

        if (is_string($key)) 
        {
            $value = mysql_real_escape_string($key);
            $sql .= "ContactValueSubtypeName = '{$value}'";
        } 
        elseif (is_int($key))
        {
            $sql .= "ContactValueSubtypeID = {$key}";
        } 
        elseif (is_object($key))
        {
            if ($key->ID())
            {
                $sql .= "ContactValueSubtypeID = " . $key->ID();
            }
            else
            {
        ...
        if ($result)
		{
            ...
            $this->p_ID = $row['ContactValueSubtypeID'];
    		$this->p_name = $row['ContactValueSubtypeName'];
    		$this->p_description = $row['ContactValueSubtypeDescription'];
    		$this->p_note = $row['ContactValueSubtypeNote'];
