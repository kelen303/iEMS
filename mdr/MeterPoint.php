...
class MeterPoint extends CAO
{
  ...
  /**
   * MeterPoint::pointType()
   *
   * @return
   */
    function pointType($pointType = null)
    {
        if (!isset($pointType)) {
            return $this->p_pointType;
        } else {
            $this->p_pointType = clone $pointType;
        }
    }
  ...
  /**
   * MeterPoint::isNetAsset()
   *
   * @return
   */
    function isNetAsset($isNetAsset = null)
    {
        if (!isset($isNetAsset)) {
            return $this->p_isNetAsset;
        } else {
            $this->p_isNetAsset = $isNetAsset;
        }
    }
   ...
   */
    function RefreshPrices()
    {
        ...
        if ($this->p_displayPriceId) {
            ...
            //echo "sql='{$sql}...<br>\n";
            //select max(IntervalValue) MaximumIntervalValue from t_priceintervals where PriceID = 3 and IntervalDate between '2012-01-05 00:05:00' and '2012-01-06 00:00:00'
            $result = mysql_query($sql, $this->sqlConnection());
            if ($row = mysql_fetch_array($result)) {
