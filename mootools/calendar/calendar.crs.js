...
var Calendar = new Class({	
	initialize: function(obj, props) {
		...
		this.calendar = new Element('div', { 
			'styles': { left: '-1000px', opacity: 0, position: 'absolute', top: '-1000px', zIndex: 30000 }
...
/* 
//mcb: remming this for now because it is trapping the 'select' for existing values.
//For this exercise, we are pushing, not pulling, to a select so we need this to go away for now.
...
*/		
	...
	display: function(cal) {
		...
		//mcb: likely need to change this to trap for our selected array
		...
		for (var i = 1; i < 43; i++) {
			...
			var cls = '';
			//mcb here is where we're setting the active css value
	...
	evaluate: function(cal) {
		...
		if ($type(arr[1]) == 'number') { cal.month = arr[1]; 4}
