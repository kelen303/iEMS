...
var Calendar = new Class({	
	...
	bounds: function(cal) {
		...
		/*mcb remming due to the way we handle selects
	...
	clicked: function(td, day, cal) {
		//mcb don't forget to put in the check for select!
		cal.val = (this.value(cal) == day) ? null : new Date(cal.year, cal.month, day); // set new value - if same then disable
		this.insertOldSchool(this.format(cal.val, 'm-d-Y'), this.format(cal.val, 'm-d-Y'));
		this.write(cal); 
...
clicked: function(cal) {
		...
		cal.els.each(function(el) {	// then we can set the value to the field
			...
			//mcb: we're checking for the 'select' field type because it needs to be processed differently

			//than a simple text field.

			if (el.getTag() == 'select') {

				this.insertOldSchool(el, this.format(cal.val, el.format), this.format(cal.val, el.format));
...
*/
		...
		el.readOnly = false; //true; klk -- override of mcb

		el.addEvent('focus', function(cal) { this.toggle(cal); }.pass(cal, this)); //mcb
	...
	//mcb: function for pushing cal values into multi-select



	insertOldSchool: function( newText, newValue){
	  ...
	  if (theSel.length == 0) {
...

		{
			...
			//we can check for dupes this way because of the selText.sort() we call below

			if(theSel.options[i].text == newText){
...
//mcb additional calendar-related utility functions



function selectCompareDates(theSel)

{

	//here we just want to scoop up all the values in the select box and get them sent to post
...
function setDates(dateRange,fromField,toField)

	{

		var selValue = dateRange.options[dateRange.selectedIndex].value;
		...
		switch(selValue) {
			...
			case 'thisWeek':
                ...
                fromDate.setDate(fromDate.getDate()-curDate.getDay()+1);
			...
			case 'lastWeek':
                ...
                fromDate.setDate(fromDate.getDate()-curDate.getDay()-6);
		...
		/** following is simply to get leading zeros added **/

        var fromMonth = (fromDate.getMonth()<9?'0':'') + (fromDate.getMonth() + 1);

        var fromDay = (fromDate.getDate()<10?'0':'') + fromDate.getDate();

        var fromYear = fromDate.getFullYear();		



        var toMonth = (toDate.getMonth()<9?'0':'') + (toDate.getMonth() + 1);

        var toDay = (toDate.getDate()<10?'0':'') + toDate.getDate();

        var toYear = toDate.getFullYear();		

		

		fromField.value= fromMonth + '-' + fromDay + '-' + fromYear;

		toField.value= toMonth + '-' + toDay + '-' + toYear;
	...
	function removeSelectedDate(theSel,selectAll)
